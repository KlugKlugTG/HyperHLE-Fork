 ...
 */
//! Paths for host files used by HyperHLE: settings, fonts, etc.
//!
//! There are three categories of files:
//!
//! * Resources bundled with HyperHLE that neither HyperHLE nor the user should
...
//! * Files the user is expected to modify, but not HyperHLE: [APPS_DIR],
//!   [USER_OPTIONS_FILE], [WALLPAPER_FILES]. These are ordinary files and are
//!   found in [user_data_base_path].
//! * Files that HyperHLE will create and modify, and the user may modify if
...
/// Name of the directory containing fonts bundled with HyperHLE.
pub const FONTS_DIR: &str = "HyperHLE_fonts";

/// Name of the file containing HyperHLE's default options for various apps.
pub const DEFAULT_OPTIONS_FILE: &str = "HyperHLE_default_options.txt";

/// macOS-only: If HyperHLE is located in a .app bundle, return the path of the
/// Resources directory. If HyperHLE is not located in a .app bundle, return
/// [None].
#[allow(dead_code)]
...
/// HyperHLE has to be able to display their license terms.
...
/// Name of the directory where HyperHLE will store sandboxed app data, e.g.
/// the `Documents` directory.
pub const SANDBOX_DIR: &str = "HyperHLE_sandbox";

/// Name of the directory where HyperHLE will store IMG_####.PNG files saved to
/// the Photo Album.
pub const PHOTO_ALBUM_DIR: &str = "DCIM/100APPLE";

/// Get a platform-specific base path needed for accessing HyperHLE's
...
pub fn user_data_base_path() -> Cow<'static, Path> {
    ...
    #[cfg(not(target_os = "android"))]
    {
        // When HyperHLE is run from a .app bundle on macOS, the user might not
        ...
        if get_macos_bundled_resources_path().is_some() {
            return Cow::from(PathBuf::from(
                sdl2::filesystem::pref_path("hyperhle.org", "HyperHLE").unwrap(),
            ));
        }
        Cow::from(Path::new("."))
    }
...
pub fn url_for_opening_user_data_dir() -> Result<String, String> {
    if std::env::consts::OS == "android" {
        ...
        Ok(format!(
            "content://org.hyperhle.android{}{}.provider/root/root",
...
/// platforms these are simply bundled with HyperHLE in a ZIP file.)
