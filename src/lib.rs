...
pub fn main<T: Iterator<Item = String>>(mut args: T) -> Result<(), String> {
    ...
    let bundle_path = if let Some(bundle_path) = bundle_path {
        ...
        echo!("No app specified, opening app picker. Use the --help flag to see command-line usage.");
    ...
    let bundle_data = fs::BundleData::open_any(&bundle_path).map_err(|e| format!("Could not open app bundle: {e}"))?;
    let (bundle, fs) = match bundle::Bundle::new_bundle_and_fs_from_host_path(bundle_data, false) {
        Ok(bundle) => bundle,
        Err(err) => return Err(format!("Application bundle error: {err}. Check that the path is to an .app directory or an .ipa file.")),
    };
    ...
    echo!("- Minimum OS version: {}", minimum_os_version.as_deref().unwrap_or("(not specified)"));
    echo!("- Required device capabilities: {}", if !required_device_capabilities.is_empty() { required_device_capabilities.join(", ") } else { "(not specified)".to_string() });
    echo!("- Device family: {}", if !device_family.is_empty() { device_family.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", ") } else { "(not specified)".to_string() });
    echo!();
    if let Some(version) = minimum_os_version.as_deref() {
        ...
        if major > 9 || (major == 9 && minor > 0) {
            echo!("Warning: app requires OS version {}. touchHLE currently aims \\
                 for iOS 2.x–9.0; newer APIs may be missing.", version);
        }
    }
    if required_device_capabilities.contains(&"opengles-3") {
        echo!("Warning: app requires OpenGL ES 3.0+ support. HyperHLE now routes EAGL OpenGL ES 3 contexts to its GLES 3 backend.");
    }
    if just_info {
        return Ok(());
    }
    fn apply_options<F: std::io::Read, P: std::fmt::Display>(file: F, path: P, options: &mut options::Options, app_id: &str) -> Result<(), String> {
        match options::get_options_from_file(file, app_id) {
            Ok(Some(options_string)) => {
                echo!("Using options from {} for this app: {}", path, options_string);
            ...
            Ok(None) => echo!("No options found for this app in {}", path),
            Err(e) => echo!("Warning: {}", e),
        }
        Ok(())
    }
    ...
    match std::fs::File::open(&user_options_path) {
        ...
        Err(err) => echo!("Warning: Could not open {}: {}", user_options_path.display(), err),
    }
    echo!();
    for option_arg in option_args {
        let parse_result = options.parse_argument(&option_arg);
    ...
    let res = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| Environment::new(bundle, fs, options.clone(), app_args.unwrap_or_default())));
    let env = match res {
        Ok(ret) => match ret {
            Ok(env) => env,
            Err(e) => {
                if options.popup_errors { window::show_error_messagebox(None, e.as_str()); }
                return Err(e);
            }
        },
        Err(e) => {
            if options.popup_errors {
                let error_string = if let Some(s) = e.downcast_ref::<&str>() { s } else if let Some(s) = e.downcast_ref::<String>() { s } else { "(non-string payload)" };
