...
//! keeps the HyperHLE runtime single-threaded and simple.
...
@implementation _HyperHLE_CFHost: NSObject
- (())dealloc {
...
fn alloc_cfhost(
    ...
    let class = env.objc.get_known_class("_HyperHLE_CFHost", &mut env.mem);
...
pub fn CFHostGetTypeID(env: &mut Environment) -> CFTypeID {
    let class: Class = env.objc.get_known_class("_HyperHLE_CFHost", &mut env.mem);
...
fn start_names(
    ...
    // Reverse DNS is rarely useful for HyperHLE; report the name we were
