...
/// Bundle is stored in crate::Environment, not here.pub bundle: Option<Bundle>,
...
// 1. CACHE CHECKif let Some(&cached) = env.framework_state.foundation.ns_bundle.bundle_cache.get(&path_str) {
release(env, this);
...
let path: id = msg![env; base stringByAppendingPathComponent:comp];autorelease(env, path)
}
...
ofType:(id)extension     // NSString*
inDirectory:(id)directory {   // NSString*
let path = path_for_resource_helper(env, this, name, nil, directory, extension);if path != nil {
return path;
}
// Try preferred languages in order.
let langs: id = msg_class![env; NSLocale preferredLanguages];
...
ofType:(id)extension      // NSString*
inDirectory:(id)directory      // NSString*
forLocalization:(id)localization { // NSString*if localization != nil {
let lproj_suffix: id = ns_string::get_static_str(env, ".lproj");
let lproj_dir: id = msg![env; localization stringByAppendingString:lproj_suffix];
...
// =========================================================================
- (id)infoDictionary {if let Some(dict) = env.objc.borrow::<NSBundleHostObject>(this).info_dictionary {
return dict;
}
...
let exec_path = env.bundle.executable_path().as_str().to_string();log!("path_for_resource_helper: empty name -> returning exec path: {}", exec_path);
return ns_string::from_rust_string(env, exec_path);
}
