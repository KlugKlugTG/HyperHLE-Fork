...
// =========================================================================
//
// HyperHLE has no HTML rendering engine. As an opportunistic fallback (see
...
fn find_chromium_binary() -> Option<PathBuf> {
    ...
    if let Ok(path) = std::env::var("HYPERHLE_CHROMIUM") {
...
fn snapshot_url_with_chromium(url: &str, width: u32, height: u32) -> Option<Vec<u8>> {
    ...
    let tmp = std::env::temp_dir().join(format!("hyperhle_uiwebview_{}.png", idx));
