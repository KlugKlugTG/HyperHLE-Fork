...
//! technology changes, or check the country code. RadekHLE has no real
...
struct CTTelephonyNetworkInfoHostObject {
    ...
    /// observe in RadekHLE), but the property must round-trip via KVC.
...
// RadekHLE has no cellular radio, so there is no radio-access technology
...
- (())setSubscriberCellularProviderDidUpdateNotifier:(id)block {
    ...
    // caller's stack; in practice RadekHLE's block runtime already heap-
