...
pub fn app_picker(options: Options) -> Result<(PathBuf, Vec<String>), String> {
    ...
    let apps: Result<Vec<AppInfo>, String> = if !apps_dir.is_dir() {
        Err(format!("The {} directory couldn't be found. Check you're running HyperHLE from the right directory.", apps_dir.display()))
    } else {
...
pub const DYLIB: crate::dyld::HostDylib = crate::dyld::HostDylib {
    ...
    path: "/.HyperHLE/AppPickerHelpers.dylib",
    aliases: &[],
...
@implementation _HyperHLE_AppPickerDelegate: NSObject
...
- (())visitWebsite {
    ...
    let url = ns_string::get_static_str(env, "https://github.com/j92580498-max/HyperHLE");
...
fn app_picker_inner(
    ...
    let delegate = env
        .objc
        .get_known_class("_HyperHLE_AppPickerDelegate", &mut env.mem);
    ...
    make_button_row(
        ...
        &[
            ...
            ("HyperHLE on GitHub", "visitWebsite"),
        ],
        None,
    );
    ...
    // If an app is picked, this loop returns. If the user quits HyperHLE, the
    // process exits.
    let app_path = loop {
        ...
        if icon_tapped != nil {
            match icon_grid_stuff.as_ref().unwrap().icon_map.get(&icon_tapped) {
                Some(&TappedIcon::App(app_idx)) => {
                    ...
                    // Ensure HyperHLE is responsive from the OS perspective,
