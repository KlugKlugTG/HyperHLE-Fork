...
extern "C" {
struct HyperHLE_Mem;
std::uint8_t HyperHLE_cpu_read_u8(HyperHLE_Mem *mem, VAddr addr, bool *error);
std::uint16_t HyperHLE_cpu_read_u16(HyperHLE_Mem *mem, VAddr addr, bool *error);
std::uint32_t HyperHLE_cpu_read_u32(HyperHLE_Mem *mem, VAddr addr, bool *error);
std::uint64_t HyperHLE_cpu_read_u64(HyperHLE_Mem *mem, VAddr addr, bool *error);
bool HyperHLE_cpu_write_u8(HyperHLE_Mem *mem, VAddr addr, std::uint8_t value);
bool HyperHLE_cpu_write_u16(HyperHLE_Mem *mem, VAddr addr, std::uint16_t value);
bool HyperHLE_cpu_write_u32(HyperHLE_Mem *mem, VAddr addr, std::uint32_t value);
bool HyperHLE_cpu_write_u64(HyperHLE_Mem *mem, VAddr addr, std::uint64_t value);
struct HyperHLE_DynarmicContext {
...
private:
  std::uint8_t MemoryRead8(VAddr vaddr) override {
    bool error;
    auto value = HyperHLE_cpu_read_u8(mem, vaddr, &error);
    if (error) {
  ...
  std::uint16_t MemoryRead16(VAddr vaddr) override {
    bool error;
    auto value = HyperHLE_cpu_read_u16(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
  ...
  std::uint32_t MemoryRead32(VAddr vaddr) override {
    bool error;
    auto value = HyperHLE_cpu_read_u32(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
  ...
  std::uint64_t MemoryRead64(VAddr vaddr) override {
    bool error;
    auto value = HyperHLE_cpu_read_u64(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
...
public:
  ...
  void swap_context(HyperHLE_DynarmicContext *context) {
    HyperHLE_DynarmicContext tmp = {cpu->Regs(), cpu->ExtRegs(), cpu->Cpsr(),
                                    cpu->Fpscr()};
  ...
  std::int32_t run_or_step(HyperHLE_Mem *mem, std::uint64_t *ticks) {
    env.mem = mem;
...
DynarmicWrapper *HyperHLE_DynarmicWrapper_new(void *direct_memory_access_ptr,
                                              size_t null_page_count) {
  return new DynarmicWrapper(direct_memory_access_ptr, null_page_count);
}
void HyperHLE_DynarmicWrapper_delete(DynarmicWrapper *cpu) { delete cpu; }

const std::uint32_t *
HyperHLE_DynarmicWrapper_regs_const(const DynarmicWrapper *cpu) {
  return cpu->regs();
}
std::uint32_t *HyperHLE_DynarmicWrapper_regs_mut(DynarmicWrapper *cpu) {
  return cpu->regs();
}

std::uint32_t HyperHLE_DynarmicWrapper_cpsr(const DynarmicWrapper *cpu) {
  return cpu->cpsr();
}
void HyperHLE_DynarmicWrapper_set_cpsr(DynarmicWrapper *cpu,
                                       std::uint32_t cpsr) {
  cpu->set_cpsr(cpsr);
}

void HyperHLE_DynarmicWrapper_swap_context(DynarmicWrapper *cpu,
                                           HyperHLE_DynarmicContext *context) {
  cpu->swap_context(context);
}

void HyperHLE_DynarmicWrapper_invalidate_cache_range(DynarmicWrapper *cpu,
                                                     VAddr start,
                                                     std::uint32_t size) {
  cpu->invalidate_cache_range(start, size);
}

std::int32_t HyperHLE_DynarmicWrapper_run_or_step(DynarmicWrapper *cpu,
                                                  HyperHLE_Mem *mem,
                                                  std::uint64_t *ticks) {
