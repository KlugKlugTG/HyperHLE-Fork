...
extern "C" {
struct RadekHLE_Mem;
std::uint8_t RadekHLE_cpu_read_u8(RadekHLE_Mem *mem, VAddr addr, bool *error);
std::uint16_t RadekHLE_cpu_read_u16(RadekHLE_Mem *mem, VAddr addr, bool *error);
std::uint32_t RadekHLE_cpu_read_u32(RadekHLE_Mem *mem, VAddr addr, bool *error);
std::uint64_t RadekHLE_cpu_read_u64(RadekHLE_Mem *mem, VAddr addr, bool *error);
bool RadekHLE_cpu_write_u8(RadekHLE_Mem *mem, VAddr addr, std::uint8_t value);
bool RadekHLE_cpu_write_u16(RadekHLE_Mem *mem, VAddr addr, std::uint16_t value);
bool RadekHLE_cpu_write_u32(RadekHLE_Mem *mem, VAddr addr, std::uint32_t value);
bool RadekHLE_cpu_write_u64(RadekHLE_Mem *mem, VAddr addr, std::uint64_t value);
struct RadekHLE_DynarmicContext {
...
private:
  std::uint8_t MemoryRead8(VAddr vaddr) override {
    bool error;
    auto value = RadekHLE_cpu_read_u8(mem, vaddr, &error);
    if (error) {
  ...
  std::uint16_t MemoryRead16(VAddr vaddr) override {
    bool error;
    auto value = RadekHLE_cpu_read_u16(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
  ...
  std::uint32_t MemoryRead32(VAddr vaddr) override {
    bool error;
    auto value = RadekHLE_cpu_read_u32(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
  ...
  std::uint64_t MemoryRead64(VAddr vaddr) override {
    bool error;
    auto value = RadekHLE_cpu_read_u64(mem, vaddr, &error);
    if (error) {
      cpu->HaltExecution(Dynarmic::HaltReason::MemoryAbort);
    }
    return value;
...
public:
  ...
  void swap_context(RadekHLE_DynarmicContext *context) {
    RadekHLE_DynarmicContext tmp = {cpu->Regs(), cpu->ExtRegs(), cpu->Cpsr(),
                                    cpu->Fpscr()};
  ...
  std::int32_t run_or_step(RadekHLE_Mem *mem, std::uint64_t *ticks) {
    env.mem = mem;
...
DynarmicWrapper *RadekHLE_DynarmicWrapper_new(void *direct_memory_access_ptr,
                                              size_t null_page_count) {
  return new DynarmicWrapper(direct_memory_access_ptr, null_page_count);
}
void RadekHLE_DynarmicWrapper_delete(DynarmicWrapper *cpu) { delete cpu; }

const std::uint32_t *
RadekHLE_DynarmicWrapper_regs_const(const DynarmicWrapper *cpu) {
  return cpu->regs();
}
std::uint32_t *RadekHLE_DynarmicWrapper_regs_mut(DynarmicWrapper *cpu) {
  return cpu->regs();
}

std::uint32_t RadekHLE_DynarmicWrapper_cpsr(const DynarmicWrapper *cpu) {
  return cpu->cpsr();
}
void RadekHLE_DynarmicWrapper_set_cpsr(DynarmicWrapper *cpu,
                                       std::uint32_t cpsr) {
  cpu->set_cpsr(cpsr);
}

void RadekHLE_DynarmicWrapper_swap_context(DynarmicWrapper *cpu,
                                           RadekHLE_DynarmicContext *context) {
  cpu->swap_context(context);
}

void RadekHLE_DynarmicWrapper_invalidate_cache_range(DynarmicWrapper *cpu,
                                                     VAddr start,
                                                     std::uint32_t size) {
  cpu->invalidate_cache_range(start, size);
}

std::int32_t RadekHLE_DynarmicWrapper_run_or_step(DynarmicWrapper *cpu,
                                                  RadekHLE_Mem *mem,
                                                  std::uint64_t *ticks) {
